...
class ApiKeyControllerTest {
    ...
    @Test
    void myKeys_redirectsOrFailsGracefullyWhenUserNotFound() throws Exception {
        Mockito.when(accountRepository.findByGoogleOidc2Id(any())).thenReturn(Optional.empty());

        mvc.perform(get("/apikeys/mykeys")
                        .with(SecurityMockMvcRequestPostProcessors.authentication(mockOAuth2Auth("missing-user"))))
                .andExpect(status().isOk())
    ...
    @Test
    void myKeys_handlesNullApiKeys() throws Exception {
        Mockito.when(accountRepository.findByGoogleOidc2Id("test-id"))
                .thenReturn(Optional.of(new Account()));
        Mockito.when(issuedApiKeyService.allApiKeys()).thenReturn(null);

        mvc.perform(get("/apikeys/mykeys")
                        .with(SecurityMockMvcRequestPostProcessors.authentication(mockOAuth2Auth("test-id"))))
                .andExpect(status().isOk())
                .andExpect(view().name("apikeys/mykeys"));
    }
    ...
    @Test
    void showGeneratedApiKey_redirectsWhenResponseValueIsNull() throws Exception {
        ResponseApiKeyOnce response = new ResponseApiKeyOnce("API-KEY", null, "message");
